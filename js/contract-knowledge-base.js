/**
 * IAM Smart Contract Knowledge Base
 * این فایل شامل تمام اطلاعات قرارداد IAM برای استفاده دستیار هوشمند در زمان آفلاین است
 */

window.IAM_KNOWLEDGE_BASE = {
    // اطلاعات کلی قرارداد
    contractInfo: {
    ...
    registration: {
        cost: {
            ...
            details: [
                "اگر تعداد کاربران زیر 1000 باشد: 100 IAM یا 150 IAM (بستگی به موجودی قرارداد)",
                "اگر بیشتر از 1000 کاربر باشد: 200 IAM"
            ]
        },
        requirements: [
    ...
    faq: {
        "چگونه می‌توانم ثبت‌نام کنم؟": {
            answer: "باید توسط یک عضو موجود معرفی شوید و هزینه ثبت‌نام (بین 100-200 IAM) را پرداخت کنید. این کار از طریق تابع registerAndActivate انجام می‌شود.",
            category: "registration"
        },
        "هزینه ثبت‌نام چقدر است؟": {
            answer: "هزینه متغیر است: اگر تعداد کاربران زیر 1000 باشد: 100 IAM یا 150 IAM (بستگی به موجودی قرارداد). اگر بیشتر از 1000 کاربر باشد: 200 IAM.",
            category: "registration"
        },
    ...
    // دستورات مفید
    usefulCommands: {
        "قیمت توکن": "برای مشاهده قیمت فعلی IAM",
...
window.searchKnowledgeBase = function(query) {
    const knowledge = window.IAM_KNOWLEDGE_BASE;
...
window.getAssistantResponse = function(userQuery) {
    ...
    // پاسخ پیش‌فرض
    return `متأسفانه پاسخ دقیقی برای سوال شما پیدا نکردم. 😕\n\nلطفاً سوال خود را به شکل دیگری مطرح کنید یا از دستورات زیر استفاده کنید:\n\n${Object.entries(window.IAM_KNOWLEDGE_BASE.usefulCommands).map(([cmd, desc]) => `• ${cmd}: ${desc}`).join('\n')}`;
};

console.log('✅ IAM Knowledge Base loaded successfully!'); 